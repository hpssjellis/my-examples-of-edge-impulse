...
#include <ei-v2-jeremy-horse-pen-unknown-v2_inferencing.h>
 ...
 Serial.println();
   ...
   // features_signal.get_data = &cutout_get_data;
    ...
    ei_printf("(DSP: %d ms., Classification: %d ms., Anomaly: %d ms.)",
        result.timing.dsp, result.timing.results.classification, result.timing.anomaly);
    ei_printf(": \n");
    ei_printf("[");
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
        ei_printf("%.5f", result.results[ix].value);
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
        ei_printf("    %s: %.5f\n", result.results[ix].label, result.results[ix].value);
   ...
   // myMessage02 = "";
    ...
    if (result.results[0].value>= 0.3){  //BLUE on first object // can tune a bit here
    ...
    if (result.results[1].value>= 0.5){  // green on second object
      digitalWrite(LEDG, LOW);   // this is the unknown
      //delay(300);
    } 
    
    if (result.results[2].value>= 0.7){   // red on third object
