...
void loop()
{
    ...
    u8g2.drawStr(0,10,"Edge Impulse");  // write something to the internal memory
...
/*   unbracket to grab an image of what the board sees   */
    ...
    for (int y=0; y < FRAME_BUFFER_COLS; y++){  // FRAME_BUFFER_COLS = 320
       for (int x=0; x < FRAME_BUFFER_ROWS; x++){ //FRAME_BUFFER_ROWS = 240
       //frame_buffer[FRAME_BUFFER_COLS * FRAME_BUFFER_ROWS]
       uint8_t myGRAY = frame_buffer[(x * y) + y];
       if (myGRAY > 60){
          u8g2.drawPixel(x, y);
       }
      } 
