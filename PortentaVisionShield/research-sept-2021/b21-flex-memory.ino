...
void setup()
{
    ...
    u8g2.begin();  // For OLED  
    u8g2.setDrawColor(1);  // regular white on black
    ...
    x1Map = map(x1, 0, 320, 0, 127);  
    x2Map = map(x2, 0, 320, 0, 127);
    y1Map = map(y1, 0, 240, 0, 63);
    y2Map = map(y2, 0, 240, 0, 63);
...
*/
   ...
   u8g2.setDrawColor(2);   // xOr add
   u8g2.drawFrame(x1Map-1, y1Map-1,   x2Map+1, y2Map+1);
 
   u8g2.setDrawColor(1);  // regular white on black   
   u8g2.drawStr(6,12,"Rocksetta");  // write something to the internal memory
