...
/* Auto white balance configuration */
  ...
  if (err != CAM_ERR_SUCCESS)
    {
      Serial.println("Camera Error white balance");
    }
  ...
  if (err != CAM_ERR_SUCCESS)
    {
      Serial.println("Camera Error Format");
    }
  ...
  // Init the cam
  //myCam.begin(CAMERA_R320x240, 30);   // portenta
...
void loop()
{
  ...
  //}
     ...
     CamImage img = theCamera.takePicture();  // spresense 
  // int myCamResult =  myCam.grab(frame_buffer); // Portenta myCamResult should be zero 
